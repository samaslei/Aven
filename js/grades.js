...
function renderTermTabContent(subject, term, gradeStats) {
  ...
  return `
    <div class="term-breakdown-container">
      <div class="term-header-row">
        ...
        <div class="term-header-right" style="display: flex; flex-direction: column; align-items: flex-end; gap: 6px;">
          <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap; justify-content: flex-end;">
            ...
            <!-- Category Layout Toggle: 1-Column vs 2-Column Grid -->
      ...
      <!-- Categories & Assessment Entries List -->
      <div class="category-list ${categoryLayout === '2-col' ? 'layout-grid-2' : ''}">
        ${categories.length === 0 ? `
        ...
        ` : categories.map(cat => {
          ...
          if (entries.length === 0) {
            return `
              <div class="category-card compact-row" data-cat-id="${cat.id}">
                <div class="cat-compact-left">
                  ...
                  <strong class="cat-compact-name" title="${safeCatName}">${cat.category}</strong>
                  ${renderWeightIndicator(cat.weight, cat.id)}
                  <span class="cat-empty-label">no entries</span>
                </div>
                <div class="cat-compact-right">
                  <button class="btn btn-ghost btn-sm btn-add-entry" data-cat-id="${cat.id}">+ Entry</button>
                  <button class="btn btn-ghost btn-sm btn-del-cat" data-cat-id="${cat.id}" style="color: var(--danger); padding: 4px 7px;" title="Delete Category">✕</button>
                </div>
              </div>
            `;
          } else {
            ...
            return `
              <div class="category-card expanded" data-cat-id="${cat.id}">
                <div class="category-card-header">
                  <div class="cat-title-group">
                    ...
                    <h4 class="cat-name" title="${safeCatName}">${cat.category}</h4>
                    ${renderWeightIndicator(cat.weight, cat.id)}
                  </div>
                  <div class="cat-header-right">
                    <div class="subject-standing-pill ${getStandingClass(catPct)}" style="font-size: 12px;">
                      <span class="standing-dot" style="background: currentColor;"></span>
                      <span>Avg: ${catPct}% (${weightedPts} pts)</span>
                    </div>
                    <button class="btn btn-ghost btn-sm btn-add-entry" data-cat-id="${cat.id}">+ Entry</button>
                    <button class="btn btn-ghost btn-sm btn-del-cat" data-cat-id="${cat.id}" style="color: var(--danger); padding: 4px 7px;" title="Delete Category">✕</button>
                  </div>
                  ...
                  <table class="cat-table">
                    <thead>
                      <tr>
                        ...
                        <th class="col-divider">Score</th>
                        <th class="col-divider">Out Of</th>
                        <th class="col-divider">Percentage</th>
                          ...
                          <tr class="grade-entry-row" data-cat-id="${cat.id}" data-ent-id="${ent.id}">
                            ...
                            <td class="col-divider">
                              <div class="editable-cell editable-score" data-field="score" data-cat-id="${cat.id}" data-ent-id="${ent.id}" data-value="${ent.score}" title="Click to edit score">
                                <span class="cell-value-text" style="font-family: var(--font-numeric); font-variant-numeric: tabular-nums; font-weight: 600;">${ent.score}</span>
                                <svg class="edit-hint-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                </svg>
                            ...
                            <td class="col-divider">
                              <div class="editable-cell editable-outof" data-field="out_of" data-cat-id="${cat.id}" data-ent-id="${ent.id}" data-value="${ent.out_of}" title="Click to edit total points">
                                <span class="cell-value-text" style="font-family: var(--font-numeric); font-variant-numeric: tabular-nums; font-weight: 600;">${ent.out_of}</span>
                                <svg class="edit-hint-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                </svg>
                            ...
                            <td class="col-divider">
                              <div class="subject-standing-pill ${getStandingClass(entPct)}" style="font-size: 12px;">
                                <span class="standing-dot" style="background: currentColor;"></span>
                                <span>${entPct}%</span>
                              </div>
                            </td>
                            <td style="text-align: right;">
                              <button class="btn btn-ghost btn-sm btn-del-entry" data-cat-id="${cat.id}" data-ent-id="${ent.id}" style="color: var(--danger); padding: 3px 6px;">
                                Delete
                              </button>
                            </td>
                          </tr>
                    ...
                    <tfoot>
                      <tr class="cat-totals-row">
                        <td>TOTALS</td>
                        <td class="col-divider total-num-cell" style="font-family: var(--font-numeric); font-variant-numeric: tabular-nums; text-align: right;">${totalScore.toFixed(1)}</td>
                        <td class="col-divider total-num-cell" style="font-family: var(--font-numeric); font-variant-numeric: tabular-nums; text-align: right;">${totalOutOf.toFixed(1)}</td>
                        <td class="col-divider total-num-cell" style="font-family: var(--font-numeric); font-variant-numeric: tabular-nums; font-weight: 700; text-align: right; color: ${getStandingColor(catPct)};">${catPct !== '—' ? `${catPct}%` : '—'}</td>
                        <td></td>
                      </tr>
                    </tfoot>
                  </table>
